...
mod tests {
    ...
    #[test]
    fn virtual_column_wheel_at_parked_top_republishes() {
        use iced::advanced::clipboard;
        use iced::advanced::layout::{Layout, Limits};
        use iced::advanced::widget::Tree;
        use iced::mouse;
        ...
        let tok = tea();
        let row_h = crate::live::TIMELINE_ROW_H;
        let viewport = 400.0;
        let heights: Vec<f32> = (0..40).map(|_| row_h).collect();
        let window = VisibleWindow::new(viewport);
        let mut el: iced::Element<'_, VisibleWindow> = virtual_column(
            ...
            None,
            |w| w,
            Some(Id::new("hud-timeline")),
            tok,
            |i| label(format!("ev {i}"), tok, A11y::new("r", Role::ListItem)),
            A11y::new("Timeline", Role::List),
        );
        let mut tree = Tree::new(el.as_widget());
        let renderer = iced::Renderer::Secondary(iced_tiny_skia::Renderer::new(
        ...
        assert!(
            !first.is_empty(),
            "first wheel into the parked top should publish for paging"
        );
        assert!(
            !second.is_empty(),
            "0.12.1 at_edge is 'is at 0/max', so a second parked-top wheel still publishes"
        );
    }
